...
async function tmdb(path, parameters = {}, allowNotFound = false) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `TMDB ${response.status} for ${path}: ${body.slice(0, 300)}`
      );
    }
...
function bestSearchResult(movie, results) {
  ...
  return results
    ...
    .map(result => {
      ...
      if (year === wantedYear) {
        score += 35;
      } else if (year && Math.abs(year - wantedYear) === 1) {
        score += 10;
      }
...
async function resolveRecord(movie) {
  ...
  if (!match || match.score < 70) {
    throw new Error(
      `No confident TMDB match for ${movie.title} (${movie.year})`
    );
  }
  ...
  return {
    ...
    character:
      ...
      person.roles
        ...
        .join(" / ") ||
      "",
    order: Number.isFinite(person.order)
      ? person.order
      : 9999,
...
function serialize(movie, mediaType, details) {
  ...
  const writers = crewCredits
    .filter(person =>
      WRITING_JOB_ORDER.has(person.job)
    )
    .sort(
      (a, b) =>
  ...
  /*
    Keep one visible Crew card per person, but preserve every relevant
    director/writing job in `jobs`. This matters when the same person is
    both director and writer on the same film.
  */
  const crew = [];

  [...directors, ...writers].forEach(person => {
    const existing = crew.find(
      item =>
        String(item.id) ===
        String(person.id)
    );
    ...
    if (existing) {
      if (
        person.job &&
        !existing.jobs.includes(person.job)
      ) {
...
for (const [index, movie] of movies.entries()) {
  process.stdout.write(
    `[${index + 1}/${movies.length}] ${movie.title}\n`
  );

  const {
    mediaType,
    details
  } = await resolveRecord(movie);
...
const banner =
  "// Generated from the TMDB API by scripts/fetch-tmdb-details.mjs.\n";

const js =
  `${banner}` +
  `window.WLW_TMDB_DETAILS = ${JSON.stringify(byTitle, null, 2)};\n` +
  `window.WLW_TMDB_DETAILS_BY_ID = ${JSON.stringify(byId, null, 2)};\n`;

fs.writeFileSync(
  new URL(
    "tmdb-details.generated.js",
    root
  ),
  js
);
...
console.log(
  `Generated TMDB details for ${movies.length} A titles.`
);

console.log(
  `${Object.keys(review).length} titles contain cast entries requiring manual gender review.`
);
